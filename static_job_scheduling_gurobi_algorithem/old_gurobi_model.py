...
class gurobi_model():
    ...
    def build_model(self, model):
        ...
        M = 1500
        # create vars
        x_list = [(i,j) for i in range(self.A) for j in range(self.A)]           # create vars x[i][j]
        x = model.addVars(x_list, vtype=GRB.BINARY, name="x")
        # z_list = [(i,j) for i in range(self.A) for j in range(self.A)]           # create vars z[i][j]
        ...
        # model.addConstrs( t[i] - t[j] - self.tasks[j]['execution_time'] >= (-M) * z[i,j] for i in range(self.A) for j in range(self.A)) # 决策变量与时间之间的关系
        # model.addConstrs( t[i] - t[j] - self.tasks[j]['execution_time'] <= M * (1 - z[i,j]) for i in range(self.A) for j in range(self.A)) # 决策变量与时间之间的关系
        model.addConstrs( x[i,j] >= self.y[j,i] + self.y[i,j] - 1 for i in range(self.A) for j in range(self.A)) # 决策变量x同样z之间的关系
        # model.addConstrs( x[i,j] <= z[i,j] for i in range(self.A) for j in range(self.A))
        # model.addConstrs( x[i,j] <= z[j,i] for i in range(self.A) for j in range(self.A))
        # model.addConstrs( gp.quicksum( self.tasks[j]['resource_load'] * x[i,j] for j in range(self.A)) <= self.R for i in range(self.A)) # 资源上限约束
        model.addConstrs( gp.quicksum( self.tasks[j]['resource_load'] * x[i,j] for j in range(self.A)) <= u for i in range(self.A)) # 最大资源负载约束
        model.addConstr( u >= self.calculate_resource_lower_bound() ) # 最小资源负载约束
        model.update()
        info = {
            "x" : x,
            "u" : u,
...
if __name__ == "__main__":
    ...
    # create gurobi model
    test_model = gurobi_model(test_instance, time_limit=4000)
